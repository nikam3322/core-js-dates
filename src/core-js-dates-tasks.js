...
function getCountDaysInMonth(month, year) {
  ...
  return Math.round((nextMonth - dateObj) / 1000 / 3600 / 24);
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  return (dateEndObj.getTime() - dateStartObj.getTime()) / 1000 / 3600 / 24 + 1;
}
...
function isDateInPeriod(date, period) {
  const dateObj = new Date(date);
  const dateStartObj = new Date(period.start);
  const dateEndObj = new Date(period.end);
  if (
    dateObj.getTime() >= dateStartObj.getTime() &&
    dateObj.getTime() <= dateEndObj.getTime()
  ) {
    return true;
  }
  return false;
...
function formatDate(date) {
  ...
  const dateObj = new Date(date);
  const result = new Intl.DateTimeFormat('en-US', options).format(
    dateObj.getTime()
  );
  return result;
}
...
function getWeekNumberByDate(date) {
  ...
  const weekNumber = Math.ceil(((dateObj - firstThursday) / 86400000 + 1) / 7);
  return weekNumber;
}
